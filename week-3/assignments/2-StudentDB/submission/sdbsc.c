...
{
    // TODO
    if (fd < 0 || !s || id < MIN_STD_ID || id > MAX_STD_ID)
    {
        return ERR_DB_FILE;
    }
    ...
    if (s->id == DELETED_STUDENT_ID || memcmp(s, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) == 0)
    {
...
{
    // TODO
    if (fd < 0 || !fname || !lname || id < MIN_STD_ID || id > MAX_STD_ID || gpa < MIN_STD_GPA || gpa > MAX_STD_GPA)
    {
        return ERR_DB_FILE;
    }
    ...
    {
        if (existing.id != 0 && memcmp(&existing, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0)
        {
...
int del_student(int fd, int id)
{
    // TODO
    student_t student = {0};
    int return_code = get_student(fd, id, &student);

    if (return_code == SRCH_NOT_FOUND)
    {
...
int count_db_records(int fd)
{
    ...
    {
        if (student.id != 0 && memcmp(&student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0)
        {
            count++;
        }
    }
...
int compress_db(int fd)
{
    ...
    if (ftruncate(tmp_fd, MAX_STD_ID * STUDENT_RECORD_SIZE) == -1) {
        close(tmp_fd);
        return ERR_DB_FILE;
    }
    ...
    while ((bytes_read = read(fd, &student, STUDENT_RECORD_SIZE)) == STUDENT_RECORD_SIZE) {
        if (student.id != 0 && memcmp(&student, &EMPTY_STUDENT_RECORD, STUDENT_RECORD_SIZE) != 0) {
            if (lseek(tmp_fd, (student.id - 1) * STUDENT_RECORD_SIZE, SEEK_SET) == -1)
            {
                printf(M_ERR_DB_WRITE);
                close(tmp_fd);
                return ERR_DB_FILE;
